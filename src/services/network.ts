...
function buildConnectionSettings(
    ...
    if (model.networkAddress.ipv4.method === 'static') {
        ...
        ].filter(dns => dns && dns.trim()) as string[];
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function applyNetworkConfiguration(networkManager: any, model: Model, skipActivation = false): Promise<NetworkApplyResult> {
    ...
    try {
        ...
        for (const connPath of connectionPaths) {
            try {
                ...
                const prefix = ifaceName
                    ? `${ONBOARDING_PROFILE_PREFIX}${ifaceName}`
