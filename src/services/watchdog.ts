...
export async function armWatchdog(testHost: string, timeoutSec: number): Promise<void> {
    try {
        await cockpit.spawn(
            ['sudo', SCRIPT_RUN_WATCHDOG, String(timeoutSec), testHost],
            { err: 'message' }
        );
    } catch (error) {
...
export async function disarmWatchdog(): Promise<void> {
    try {
        await cockpit.spawn(
            ['sudo', 'systemctl', 'stop', 'cockpit-system-onboarding-watchdog.timer'],
            { err: 'message' }
        );
    } catch {
    ...
    try {
        await cockpit.spawn(
            ['sudo', 'systemctl', 'stop', 'cockpit-system-onboarding-watchdog.service'],
            { err: 'message' }
        );
    } catch {
