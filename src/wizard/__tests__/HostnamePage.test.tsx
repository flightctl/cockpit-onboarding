...
describe('validateHostname - RFC 1123 Validation', () => {
    ...
    describe('Label length validation (max 63 characters per label)', () => {
        ...
        test('validates each label in FQDN', () => {
            ...
            const label64 = 'a'.repeat(64);
            expect(validateHostname(`${label64}.example.com`)).toBe('Each hostname label must be 63 characters or less');
        });
    });
    ...
    describe('Character restrictions', () => {
        ...
        test('rejects label starting with hyphen', () => {
            ...
            expect(validateHostname('web.-server.com')).toBe('Each hostname label must start with an alphanumeric character');
        });
        ...
        test('rejects label ending with hyphen', () => {
            ...
            expect(validateHostname('server-.example.com')).toBe('Each hostname label must end with an alphanumeric character');
        });

        test('rejects special characters', () => {
            expect(validateHostname('server_01')).toBe('Hostname can only contain letters, numbers, and hyphens');
            expect(validateHostname('server@example.com')).toBe('Hostname can only contain letters, numbers, and hyphens');
