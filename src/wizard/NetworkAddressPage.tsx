                ...
                <StackItem>
                    <Alert variant="warning" isInline title={_("Connected through this interface")}>
                        {model.networkInterface.interfaceType === 'wifi'
                            ? _("Applying these changes will disconnect your browser session. The wizard will continue in the background and roll back if enrollment fails.")
                            : _("Applying these changes will disconnect your browser session. After applying, unplug the ethernet cable from this device and connect it to your production network. The wizard will wait up to 5 minutes for the new connection before rolling back.")}
                    </Alert>
                </StackItem>
            )}
            <StackItem>
...
export const NetworkConfigIPv4: React.FunctionComponent<{ isSetupInterface?: boolean }> = ({ isSetupInterface = false }) => {
    ...
    const checkIpAvailability = async () => {
        ...
        try {
            await cockpit.spawn(
                ['sudo', SCRIPT_CHECK_NETWORK, 'check-ip', interfaceName, ipAddress],
                { err: 'message' }
            );
    ...
    const checkGatewayReachability = async () => {
        ...
        try {
            await cockpit.spawn(
                ['sudo', SCRIPT_CHECK_NETWORK, 'check-gateway', interfaceName, gateway],
                { err: 'message' }
            );
    ...
    const setSubnetMask = (subnetMask: string) => {
        ...
        if (!error && model.networkAddress.ipv4.gateway && model.networkAddress.ipv4.address) {
            const gwError = validateIPv4(model.networkAddress.ipv4.gateway) ||
                validateIPv4GatewaySubnet(model.networkAddress.ipv4.address, model.networkAddress.ipv4.gateway, subnetMask);
    ...
    const setGatewayIp = (gateway: string) => {
        ...
        const subnetError = !formatError && model.networkAddress.ipv4.address && model.networkAddress.ipv4.subnetMask
            ? validateIPv4GatewaySubnet(model.networkAddress.ipv4.address, gateway, model.networkAddress.ipv4.subnetMask)
            : null;
...
export const NetworkConfigIPv6: React.FunctionComponent<{ isSetupInterface?: boolean }> = ({ isSetupInterface = false }) => {
    const { model, updateNestedModel } = useModelContext();
    ...
    const checkIpv6Availability = async () => {
        ...
        try {
            await cockpit.spawn(
                ['ping', '-6', '-c', '2', '-W', '3', '-I', interfaceName, bare],
                { err: 'message' }
            );
    ...
    const checkIpv6Gateway = async () => {
        ...
        const target = gateway.toLowerCase().startsWith('fe80:')
            ? `${gateway}%${interfaceName}`
            : gateway;
        try {
            await cockpit.spawn(
                ['ping', '-6', '-c', '2', '-W', '3', '-I', interfaceName, target],
                { err: 'message' }
            );
    ...
    const setGatewayIpv6 = (gateway: string) => {
        ...
        const subnetError = !formatError && gateway.trim() && model.networkAddress.ipv6.address
